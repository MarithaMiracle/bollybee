...
export async function generateMetadata({
  ...
  return {
    title: category.name,
    description: category.description ?? `Shop ${category.name} at Bollybee Fragrance Lab.`,
  };
}
...
export default async function CategoryShopPage({ params, searchParams }: CategoryPageProps) {
  ...
  return (
    <>
      ...
      <h1 className="font-display text-3xl sm:text-4xl md:text-5xl">{category.name}</h1>
